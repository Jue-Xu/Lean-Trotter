...
theorem lie_trotter_error_rate (A B : 𝔸) :
  ...
  have h_max : max ‖P‖ ‖Q‖ ≤ Real.exp ((‖A‖ + ‖B‖) / n) := by
    ...
    have h_P : ‖P‖ ≤ Real.exp ((‖A‖ + ‖B‖) / n) := by
      calc ‖P‖ = ‖exp 𝕂 ((n : 𝕂)⁻¹ • A) * exp 𝕂 ((n : 𝕂)⁻¹ • B)‖ := rfl
        _ ≤ ‖exp 𝕂 ((n : 𝕂)⁻¹ • A)‖ * ‖exp 𝕂 ((n : 𝕂)⁻¹ • B)‖ := norm_mul_le _ _
        _ ≤ Real.exp ‖(n : 𝕂)⁻¹ • A‖ * Real.exp ‖(n : 𝕂)⁻¹ • B‖ := by
            gcongr
            · exact norm_exp_le ((n : 𝕂)⁻¹ • A)
            · exact norm_exp_le ((n : 𝕂)⁻¹ • B)
    ...
    have h_Q : ‖Q‖ ≤ Real.exp ((‖A‖ + ‖B‖) / n) := by
      calc ‖Q‖ = ‖exp 𝕂 ((n : 𝕂)⁻¹ • (A + B))‖ := rfl
        _ ≤ Real.exp ‖(n : 𝕂)⁻¹ • (A + B)‖ := norm_exp_le _
  ...
  calc ‖P ^ n - Q ^ n‖
    ...
    _ ≤ (2 * ‖A‖ * ‖B‖ * Real.exp (2 * (‖A‖ + ‖B‖)) + 1) / n := by
        ...
        exact (div_le_div_right hn_pos).mpr h_num

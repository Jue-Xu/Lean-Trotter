...
\label{sec:childs-heuristic}
	%\claude{CORRECTNESS (2026-07-17): this subsection was titled ``Why Childs's prefactors are heuristic'', and the abstract/introduction/conclusion called the $\alpha_i$ ``heuristic prefactors'' --- suggesting the bound itself lacked rigor. In fact their Prop.~J.1 is a rigorous proposition; ``heuristic'' is their own word for the tightening \emph{strategies}, and what is unproven is tightness only. Retitled and reworked; the tightness quotation moved to the opening paragraph. Also fixed the citation numbers: the manuscript cited this result as ``Proposition~7'' and the second-order tight bound as ``Proposition~6'', but the arXiv-v3 numbering (verified against the compiled PDF) is Prop.~J.1 and Prop.~16 respectively.}
...
fixed choice, including ours.
	%\claude{CORRECTNESS (2026-07-17, second pass): previously ``Childs's specific choice is one projection'' --- the same category error corrected at the $R_5$ display in \cref{apd:tighter}: the $\alpha_i$ are triangle-inequality bound coefficients from the factored Duhamel integral, not signed coordinates of $R_5$. Also the ``minimum-$\ell^1$'' objective is weighted by $\norm{C_i}$ and instance-dependent, so it is not an ordinary coefficient $\ell^1$ minimization.}
...
precisely the root of
	%\claude{CORRECTNESS: this previously read $E_3 = \tfrac{1}{24}([B,[B,A]] - [A,[A,B]])$, i.e.\ coefficient $1/24$ on $\comm{B}{\comm{B}{A}}$. That is wrong by a factor of~2, and it contradicted the paper's own \cref{eq:comm-strang}, whose coefficients are $1/12$ and $1/24$. Lean-BCH's \texttt{symmetric\_bch\_cubic\_poly} (\texttt{RemainderBounds.lean:7339}) is $-\tfrac{1}{24}\comm{a}{\comm{a}{b}} + \tfrac{1}{12}\comm{b}{\comm{b}{a}}$, which is the standard symmetric-BCH value. The $\sum_i c_i^3 = 0$ argument is unaffected, since $E_3$ factors out regardless of its internal coefficients.}
\begin{equation}
...
structure---any complete normed algebra suffices.
	%\claude{CORRECTNESS: this paragraph previously displayed the bound $\norm{S_4(t)-e^{t(A+B)}} \le t^5\sum\gamma_i\norm{C_i}$ and attributed it to \texttt{norm\_suzuki4\_level3\_bch}. That theorem's actual statement was $\exists C \ge 0,\ \norm{\cdot} \le C t^5$ --- the $\gamma_i$ occurred nowhere in its type, only as a witness chosen inside the proof, so the displayed equation was not what was formalized. \texttt{norm\_suzuki4\_level3\_explicit} (new) does state it, modulo the honest $K' t^6$ remainder that the exp-Lipschitz lift genuinely incurs. The weak $\exists C$ form is retained under the old name as a corollary.}
...
optimality---of either family of coefficients---remains unknown.
	%\claude{CORRECTNESS (2026-07-17, third pass): this block was headed ``Childs's bound is not tight uniformly either'', called the uniform-tightness worry ``unfounded'', and said the quoted admission ``closes the matter''. ``Not proven tight'' does not imply ``not tight''. Reframed around what is actually proved (local leading-coefficient gap) with uniform optimality explicitly left open.}
...
\leanref{Suzuki4ViaBCH.lean}{1099}{bchR7UniformConstant}
and the bound
\begin{equation}
  \norm{R_7} \;\le\; K \cdot \max(\norm{A}, \norm{B})^7,
\end{equation}
encoded as \leanref{Suzuki4ViaBCH.lean}{1105}{bchR7Bound}.
...
\leanref{Suzuki4ViaBCH.lean}{1404}{norm\_suzuki4\_le\_childs\_near\_zero}
...
rather than Level~4, it needs neither anti-Hermiticity nor a C*-algebra.
	%\claude{CORRECTNESS: this paragraph replaces one that cited a lemma \texttt{norm\_suzuki4\_level4\_le\_childs\_when\_small}, now deleted from the Lean source because it was \emph{vacuous}: its side condition constrained the very $C$ that its own conclusion existentially quantified, so a prover could always return a $C$ large enough to falsify the hypothesis and discharge the implication empty --- it was provable with no BCH content at all. The threshold inequality it displayed (formerly \texttt{eq:t-threshold}) is uncheckable for the same reason and has been dropped, along with the ``orders of magnitude tighter'' estimate that was read off it.}
...
$8.6$--$64\times$ tightening of the $\gamma_i$ over the $\alpha_i$ \emph{would}
translate into proportionally fewer Trotter steps in the small-$t$
regime---the
...
\cref{eq:level3-sharp} avoids.
	%\claude{CORRECTNESS (2026-07-17): previously ``The existential $\delta$ is intrinsic rather than an artifact'' here, and ``unavoidable in a general normed algebra'' above \cref{eq:level4}. Unsupported: the $\delta$ comes from the BCH convergence radius, not from norm growth --- Duhamel-route bounds (Childs et al.\ Prop.~J.1; our \cref{sec:s4-convergence}) need no such restriction. The exponential-growth observation rules out only global \emph{polynomial} right-hand sides. Reframed in both places as a limitation of the logarithmic/BCH proof route.}
	%\claude{CORRECTNESS: the previous version of this paragraph claimed that feeding $\norm{C_i}, \norm{A}, \norm{B}$ and a step size $t < \delta$ into \cref{eq:level4} ``returns a certified numerical upper bound''. It does not: \texttt{norm\_suzuki4\_level4\_uniform} reads $\exists \delta > 0, \exists C \ge 0, \forall \tau < \delta, \ldots$, so no number is extractable at any given $t$, and the $t^5$ coefficient it certifies is $C \sum \gamma_i \norm{C_i}$ with $C$ unknown --- strictly weaker than Level~3's. Restoring the resource-estimation claim would require re-proving L4 with an \emph{explicit} $C$ and $\delta$ (the exp-Lipschitz constant is computable: it is $\exp(\delta \cdot \mathrm{const})$; nothing but effort blocks it).}
...
C\textsuperscript{*}-algebra (\cref{sec:s4-convergence}).
	%\claude{CORRECTNESS: this table previously named \texttt{norm\_suzuki4\_childs\_form} (L1), \texttt{norm\_suzuki4\_level2\_bch} (L2) and \texttt{norm\_suzuki4\_level3\_bch} (L3) for the three Forms. None of those three theorems states its Form: all conclude only $\exists C \ge 0,\ \norm{\cdot} \le C\,\tau^5$, with the $\alpha_i$, unit and $\gamma_i$ coefficients appearing nowhere in their types (they were witnesses chosen inside the proofs). Worse, \texttt{norm\_suzuki4\_childs\_form} is a \emph{different} theorem from the intended \texttt{..\_via\_level3}: it is conditional on an unproved \texttt{w4Deriv} residual hypothesis and it carries \texttt{star A = -A}, \texttt{star B = -B} plus the full C*-typeclass stack --- so the caption's claim that ``L1--L3 hold in any complete normed algebra'' was false for it. The rows now cite the theorems that do carry the coefficients in the type; with those, the caption's hypothesis claim is true. The weak $\exists C$ forms are retained under the old names as corollaries.}}
}
\label{tab:levels}
\end{table}
...
$S_4$ (\leanref{Suzuki4.lean}{375}{suzuki4\_error\_rate\_sq}), which holds for
...
an $\exists C$ bound such as \cref{eq:s4-quartic} cannot express.
	%\claude{This subsumes \cref{eq:s4-quartic} in content (same $O(1/n^4)$ order, but with an explicit commutator-scaled leading coefficient). Consider promoting \eqref{eq:s4-quartic-comm} to the headline of this subsection and demoting \eqref{eq:s4-quartic} to a remark, since the latter is now just the order statement.}
...
arbitrarily large coefficients.
	%\claude{CORRECTNESS: this last clause previously read ``any valid projection is strictly smaller than Childs's on all 8 coordinates'', which is false --- the projections form a two-parameter affine family and are unbounded. What is proved is $\abs{\gamma_i} \le \alpha_i$ for \emph{our} $\gamma$.}
...
termwise inequality $\gamma_i \le \alpha_i$. The optional L4 small-$\tau$
($\tau^7$) refinement composes the new bridge corollary
...
its transcription into Lean are \emph{not} themselves machine-checked. The
two surviving Lean-BCH septic axioms each
...
\end{itemize}
	%\claude{CORRECTNESS: the L4 bullet previously read ``a rigorous uniform finite-$t$ bound \dots strictly tighter than Childs in the Trotter regime (by orders of magnitude for small~$t$)''. \texttt{norm\_suzuki4\_level4\_uniform} is $\exists \delta > 0, \exists C \ge 0, \forall \tau < \delta$: it is neither uniform in $t$ nor numerically comparable to Childs's, since $C$ is unknown. The L3 bullet's ratio range was $9\times$--$64\times$; $0.0057/0.000663 = 8.6\times$ is the true minimum, and the two vanishing $\gamma_i$ are not in any finite ratio at all.}

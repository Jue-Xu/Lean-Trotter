...
\end{equation}
is now \emph{fully proved} at the project level: \texttt{\#print axioms
norm\_suzuki4\_level3\_bch} returns only \texttt{[propext,
Classical.choice, Quot.sound]}. The intermediate bridge theorem
...
\texttt{d455ff0}) with the per-$i$ numerical bounds
...
$[0, \delta)$. The existential-$\delta$ shape is unavoidable in a
general normed algebra: $\norm{S_4(t)}$ and $\norm{e^{t(A+B)}}$ both
grow as $\exp(t \cdot \mathrm{const})$, so a polynomial right-hand
side can only dominate their difference once the exp-Lipschitz
inflation factor $C$ is absorbed into the bound on a small-$t$
interval.
...
through two deeper Lean-BCH septic stepping stones described in
\cref{sec:caveats}) composes with exp-Lipschitz on the Lean-Trotter
...
\paragraph{Why Level 4 matters.}
L1--L3 are leading-order statements; L4 is the bound one actually
plugs a specific $t$ into for resource estimation. Feeding the eight
commutator norms $\norm{C_i}$, the operator norms $\norm{A}, \norm{B}$,
and a chosen step size $t < \delta$ into \cref{eq:level4} returns a
certified numerical upper bound on $\norm{S_4(t) - e^{t(A+B)}}$, and
the 9--64$\times$ tightening of the leading coefficient then propagates
directly into fewer Trotter steps---hence fewer quantum gates---for a
given target accuracy. The existential-$\delta$ qualification is not a
defect: in a general normed algebra both $\norm{S_4(t)}$ and
$\norm{e^{t(A+B)}}$ grow as $\exp(t \cdot \mathrm{const})$, so no
polynomial-in-$t$ bound can hold uniformly for all $t \ge 0$; the
multiplier $C$ absorbs that exponential inflation on the small-$t$
interval where it is close to~1. Beyond the immediate numerical
payoff, L4 demonstrates that the CAS-assisted BCH pipeline
\emph{iterates}: an analogous $R_9$ extension is mechanical and would
shrink the $t^8$ tail term by another two BCH orders.
...
side---while L2/L3/L4 are Lean theorems composing the corresponding
Lean-BCH bridge corollaries with exp-Lipschitz lifts. As of Lean-BCH
revision \texttt{d455ff0} (2026-05-19), L1/L2/L3 are axiom-free
(\texttt{\#print axioms} returns only the standard foundational axioms).
L4 transitively depends on two surviving Lean-BCH septic axioms
(\texttt{symmetric\_bch\_septic\_sub\_poly\_axiom},
\texttt{norm\_septic\_match\_residual\_le\_axiom}) that gate the
$\tau^7$ extension only.
...
(\leanref{Suzuki4Convergence.lean}{155}{suzuki4\_total\_error\_quartic},
\leanref{Suzuki4Convergence.lean}{254}{suzuki4\_convergence\_quartic}).
...
(\leanref{Suzuki4Convergence.lean}{308}{suzuki4\_convergence\_quartic\_suzukiP}).
...
(\leanref{Suzuki4BchBound.lean}{385}{exists\_norm\_s4Func\_sub\_exp\_le\_t5}),
...
\Cref{eq:s4-quartic} is weaker in \emph{form} than the first-order bound of
\cref{sec:assembly}, which holds for every $n \ge 1$ with the explicit constant
$C = 2\norm{A}\norm{B}e^{\norm{A}+\norm{B}} + 1$. Here both $C$ and $N$ are
existential: $N$ marks where the step size enters the BCH regime
$\abs{\tau} < \delta$, and $C$ is inherited from the constants of the
single-step BCH chain, which the present Lean-BCH interface does not expose
explicitly. The trade is deliberate. The generic $O(1/n^2)$ rate for $S_4$
(\leanref{Suzuki4.lean}{376}{suzuki4\_error\_rate\_sq}) holds for all $n$ with
an explicit constant and \emph{no} cubic condition; fourth order is precisely
what the extra hypothesis $4p^3 + (1-4p)^3 = 0$ buys.
...
(\leanref{Suzuki4Convergence.lean}{353}{suzuki4Step\_eq\_suzuki4Exp}); the four
...
\eqref{eq:s4-quartic} transfers verbatim to \texttt{suzuki4Step}
(\leanref{Suzuki4Convergence.lean}{363}{suzuki4Step\_total\_error\_quartic}), so
the improvement from $O(1/n^2)$ to $O(1/n^4)$ is recorded on one and the same
object rather than inferred across two definitions.
...
Lean-Trotter carries zero own theorem-level BCH-interface axioms. The
L1, L2, L3 bounds are \emph{fully proved at the project level}:
\texttt{\#print axioms norm\_suzuki4\_childs\_form\_via\_level3},
\texttt{\#print axioms norm\_suzuki4\_level2\_bch}, and \texttt{\#print
axioms norm\_suzuki4\_level3\_bch} each return only \texttt{[propext,
Classical.choice, Quot.sound]} as of Lean-BCH revision~\texttt{d455ff0}
(2026-05-19), which discharged the previous B1.c quintic axiom upstream.
...
termwise inequality $\gamma_i \le \alpha_i$. The optional L4 finite-$t$
uniform refinement composes the new bridge corollary
\texttt{suzuki5\_log\_product\_septic\_at\_suzukiP}, which still
transitively depends on two private axioms inside the companion
\href{https://github.com/Jue-Xu/Lean-BCH}{Lean-BCH} project:
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \texttt{BCH.symmetric\_bch\_septic\_sub\_poly\_axiom}:
    the symmetric BCH septic Taylor bridge---supplies the deg-7
    remainder bound on the three-factor symmetric BCH expansion.
  \item \texttt{BCH.norm\_septic\_match\_residual\_le\_axiom}:
    bounds the matching residual between the explicit Childs-7-basis
    R$_7$ polynomial and the deg-7 part of the five-factor palindromic
    BCH log at Suzuki~$p$.
\end{itemize}
These two stepping stones gate only the $\tau^7$ uniform extension
(L4), not the core tight 4th-order bound. The previously axiomatized
...
and \Mathlib's \texttt{iteratedDeriv\_exp\_smul\_mul\_at\_zero}. Axiom
...
$\mathbb{K} = \mathbb{R}$. The $\gamma_i$ and $K$ values are
``machine-checked mathematically'' (via the Python scripts, whose
output is reproducible); the two surviving Lean-BCH septic axioms each
have documented multi-session discharge roadmaps in the upstream
\texttt{BCH/} directory.
...
the project repository. Levels L1, L2, L3 are now fully Lean-verified
without any project-side axioms (Lean-BCH revision \texttt{d455ff0}, see
\cref{sec:caveats}); only Level~4's $\tau^7$ uniform extension still
depends on two surviving Lean-BCH-private septic axioms that will be
closed as the upstream B1.c discharge template is extended one BCH
order higher. The three-factor symmetric BCH interface on which L3/L4
depend is imported from Lean-BCH as a git dependency and is no longer

...
\leansrc{Suzuki4ViaBCH.lean}              & 1\,294 & SLICE~3 wiring + L1/L2/L3/L4 BCH-derived bounds \\
\leansrc{Suzuki4Convergence.lean}         & 380  & Total error $O(1/n^4)$ + convergence (\cref{sec:s4-convergence}) \\
\midrule
\textbf{Total} & \textbf{12\,186} & \textbf{0 \texttt{sorry} axioms,
...
all $n$ with an explicit constant and no cubic condition;
\texttt{Suzuki4Convergence.lean} upgrades the \emph{same} object to $O(1/n^4)$
under the Suzuki cubic condition (\cref{sec:s4-convergence}).}
\label{tab:files}
\end{table}
...
finite-$t$ uniform refinement (see \cref{sec:caveats}).

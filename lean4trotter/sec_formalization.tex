...
\leansrc{Suzuki4ChildsForm.lean}          & 216  & Childs arXiv Prop.~J.1 framework \\
\leansrc{Suzuki4BchBound.lean}            & 496  & SLICE~1: single-step $O(\abs{\tau}^5)$ bound \\
\leansrc{TaylorMatch.lean}                & 331  & SLICE~2: generic Taylor-match-from-norm \\
\leansrc{Suzuki4ViaBCH.lean}              & 1\,419 & SLICE~3 wiring + L1/L2/L3/L4 BCH-derived bounds \\
...
\midrule
\textbf{Total} & \textbf{14\,180} & \textbf{0 \texttt{sorry} axioms,
...
The development builds against Lean~4.29.0-rc8 and \Mathlib revision
\texttt{06a46dae}.
The companion
...
The dependency is pinned at revision~\texttt{05e8c52} (2026-07-28).
The earlier revision~\texttt{d455ff0} closed the previous B1.c
(\texttt{symmetric\_bch\_quintic\_sub\_poly\_axiom}) dependency;
the current pin also discharges the two former septic stepping stones
for Level~4. Consequently both the single-step $O(\abs{\tau}^5)$
payoff lemma anchoring \leansrc{Suzuki4BchBound.lean} and the local
$\tau^7$ refinement depend only on \Lean's standard foundational
axioms (see \cref{sec:caveats}). A clean build must compile the
Lean-BCH dependency and can be substantially longer than an incremental
build; \Mathlib oleans can be cached with \texttt{lake exe cache get}.

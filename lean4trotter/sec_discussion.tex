...
We used LLM-based coding assistants (Claude Code, Anthropic, and Codex,
OpenAI) to accelerate formalization and review, in the same general direction
as recent work on agentic autoformalization~\cite{ren2026merlean}.
...
\subsection{Trust base and axiom status}
\label{sec:axiom-status}

Lean-Trotter carries zero own theorem-level BCH-interface axioms.
Axiom inspection (\texttt{\#print axioms}) on every release
headline---\texttt{lie\_trotter}, \texttt{symmetric\_lie\_trotter}, the
commutator-scaling bounds, the abstract $S_4$ $O(t^5)$ identity
(\texttt{bch\_iteratedDeriv\_s4Func\_order4}), Levels~1--3, and the
Level~4 local small-$\tau$ refinement~\eqref{eq:level4}---returns only
\Lean's standard foundational axioms \texttt{[propext,
Classical.choice, Quot.sound]}. The dependency is pinned at Lean-BCH
revision \texttt{05e8c52} (2026-07-28), where the former septic
stepping stones \texttt{symmetric\_bch\_septic\_sub\_poly\_axiom} and
\texttt{norm\_septic\_match\_residual\_le\_axiom} have proved
replacements. Thus L4 no longer extends the trust base beyond the
foundational axioms used by the rest of the development.

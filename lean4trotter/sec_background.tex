...
\]
achieves $O(t^5)$ local error and, under the cubic condition
$4p^3 + (1-4p)^3 = 0$ that this $p$ satisfies, $O(1/n^4)$ global
convergence (\cref{sec:s4-convergence}).
...
$\norm{A}\norm{B}$. At first order, for $t \ge 0$,
...
symmetry, and---in the unitary setting (skew-adjoint $A, B$), again for
$t \ge 0$---the error is controlled by double commutators:
...
Closest to our topic, the QBlue verified compiler for Hamiltonian
simulation~\cite{liVerifiedCompilerQuantum2025} mechanizes, in Rocq, a
first-order Trotter error bound; its
\href{https://doi.org/10.5281/zenodo.15852130}{public artifact} also contains
a declared second-order Suzuki error bound. Our work extends this program to
abstract norm convergence in complete normed algebras: to our knowledge it
provides the first kernel-checked proof, with no project-specific axioms, of
\emph{norm convergence} of the Lie--Trotter sequence for arbitrary elements
of a complete normed algebra, together with commutator-scaled and
axiom-audited higher-order bounds.

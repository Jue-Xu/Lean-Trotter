...
The entire development comprises 31 Lean source files totaling over 11\,400 lines,
with zero \texttt{sorry} axioms. Three BCH-interface axioms remain, all
encoding five-factor palindromic BCH log-expansion facts for the $S_4$
integrator at orders $t^5$ and $t^7$; the three-factor symmetric BCH
identities and the two-factor palindromic quintic remainder are imported
directly from the companion
\href{https://github.com/Jue-Xu/Lean-BCH}{Lean-BCH} project (fully
sorry-free as of revision~\texttt{c71d8f2}).
Axiom inspection confirms that the headline theorems (Lie--Trotter,
Strang, commutator scaling, and the abstract $S_4$ $O(t^5)$ identity)
depend only on the standard foundational axioms \texttt{[propext,

...
\begin{enumerate}
  ...
  \item \textbf{Commutator-scaling bounds} (\cref{sec:commutator}):
    ...
    \eqref{eq:comm-first} holds for $t \ge 0$ in any complete normed algebra;
    \eqref{eq:comm-strang} additionally assumes skew-adjoint
    $A, B$ in a C*-algebra, so that $\norm{e^{tA}} = 1$.

  \item \textbf{Tighter Strang bound via norm-of-difference}: a
    second-order bound
    ...
    triangle-inequality form of \eqref{eq:comm-strang} cannot see. Its
    leading coefficient is machine-checked never to exceed that of
    \eqref{eq:comm-strang} (\texttt{norm\_D\_le\_sum\_of\_norms}), and is
    strictly smaller whenever the two double commutators partially cancel in
    $D$; a numerical sweep over standard spin chains finds strictly positive
    gains of $9$--$50\%$ in every tested configuration (\cref{apd:tighter}).

  \item \textbf{Fourth-order Suzuki $S_4$ with CAS-assisted BCH bounds}
    (\cref{sec:suzuki4}):
    four formalized order-$t^5$ bounds for
    $\norm{S_4(t) - e^{t(A+B)}}$, culminating in a replacement of the 2021
    prefactors of Childs et al.---rigorous upper bounds not proven
    tight---by CAS-certified coefficients smaller term by term by
    8.6$\times$--64$\times$ (two of the eight vanish identically); the
    comparison is machine-checked at the leading $t^5$ coefficient, and the
    full bound provably beats Childs et al.'s on some neighbourhood of
    zero whenever the compared commutators do not all vanish. An
    existential-$\delta$ small-$\tau$ refinement extends the CAS
  ...
  \item \textbf{Fourth-order convergence of $S_4$} (\cref{sec:s4-convergence}):
    ...
    $\norm{(S_4(t/n))^n - e^{t(A+B)}} \le C/n^4$ for all $n$ beyond a threshold
    $N$, and hence $(S_4(t/n))^n \to e^{t(A+B)}$. Compounding the \emph{sharp}
    step bound instead gives the same rate with an explicit, commutator-scaled
    leading coefficient. Together with items~1
    and~2 this completes a verified convergence hierarchy
    $O(1/n) \to O(1/n^2) \to O(1/n^4)$.
\end{enumerate}

The entire development comprises 41 Lean source files totaling over 14\,000 lines,
...
exp-Lipschitz lifts. At the current Lean-BCH pin \texttt{05e8c52}
(2026-07-28), both the earlier B1.c quintic assumption and the two
former septic stepping stones have been discharged. Thus all
$\tau^5$ and $\tau^7$ Suzuki~$S_4$ headlines---together with the
standard Lie--Trotter, Strang, and commutator-scaling theorems---depend
only on the standard foundational axioms \texttt{[propext,
Classical.choice, Quot.sound]}. In particular, axiom inspection on
\texttt{norm\_suzuki4\_childs\_explicit} and
\texttt{norm\_suzuki4\_le\_childs\_near\_zero} (the Level~1 Childs-form
statements), \texttt{norm\_suzuki4\_level3\_explicit} (Level~3, with the
certified $\gamma_i$ in the statement), \texttt{norm\_suzuki4\_level2\_explicit}
(Level~2 unit coefficients), the abstract $S_4$ $O(t^5)$ identity
(\texttt{bch\_iteratedDeriv\_s4Func\_order4}),
\texttt{norm\_suzuki4\_level4\_uniform}, and \texttt{lie\_trotter}
returns the three foundational axioms only. The former septic
assumptions \texttt{symmetric\_bch\_septic\_sub\_poly\_axiom} and
\texttt{norm\_septic\_match\_residual\_le\_axiom} have theorem-level
replacements in the pinned companion
\href{https://github.com/Jue-Xu/Lean-BCH}{Lean-BCH} project. The
three-factor symmetric BCH
...
\begin{itemize}
  \item To our knowledge, the \emph{first kernel-checked proof, with no
    project-specific axioms, of norm convergence} of the Lie--Trotter sequence
    for arbitrary elements of a complete normed algebra---the $n \to \infty$
    limit statement---together with an axiom-audited hierarchy of higher-order
    product-formula bounds (Strang, Suzuki $S_4$) and multi-operator
    extensions. The \href{https://doi.org/10.5281/zenodo.15852130}{public QBlue
    Rocq artifact} contains first-order Lie--Trotter and second-order Suzuki
    error-bound declarations in its verified-compiler development~\cite{liVerifiedCompilerQuantum2025}.
  ...
  \item Machine-checked \emph{commutator-scaling error bounds} at first and
    ...
    theoretical literature---and a tighter-Strang refinement
    via norm-of-difference (\cref{apd:tighter}).

  \item \emph{CAS-assisted BCH bounds for Suzuki $S_4$}: four machine-checked
    bounds---including a replacement for the Childs (2021) prefactors that
    is tighter at leading order, and an existential-$\delta$ small-$\tau$
    bound via an order-$\tau^7$ correction. The transcribed CAS constants
    are validated in Lean by \texttt{nlinarith} inequalities; the CAS
    computation itself is not machine-checked.

  \item Identification and filling of several \emph{gaps in \Mathlib}, notably
    the inequality $\norm{e^a} \le e^{\norm{a}}$ for general Banach algebras.
\end{itemize}

To our knowledge, no prior proof-assistant development contains this
Lie--Trotter norm-convergence theorem for arbitrary elements of a complete
normed algebra. QBlue's public Rocq artifact does contain first-order
Lie--Trotter and second-order Suzuki error-bound declarations in a
verified-compiler setting~\cite{liVerifiedCompilerQuantum2025}
(\cref{sec:bg-lean}); our novelty claim is therefore narrowly about this
norm-convergence theorem, its generality, and the explicitly audited trust
base, not the mere presence of a higher-order theorem statement.
...
theorem---which is a limit statement, not an algebraic identity---is the
theorem established here.

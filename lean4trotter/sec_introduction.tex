...
\begin{enumerate}
  ...
  \item \textbf{Fourth-order Suzuki $S_4$ with CAS-assisted BCH bounds}
	...
	%\claude{CORRECTNESS (2026-07-17): (i) ``strictly smaller exactly when positively aligned'' stated an iff that is false outside the proportional model --- strict triangle inequality does not require scalar alignment, and the sweep (claude/strang\_alignment\_sweep.md) finds gains even at zero/negative alignment proxy; reworded to ``partially cancel'' with the measured 9--50\% range. (ii) ``Childs's heuristic prefactors ... machine-checked never to be weaker'': their Prop.~J.1 is rigorous (only tightness unproven), and Lean's comparison is at the leading coefficient, with full-bound domination near zero under the strict-gap hypothesis of \texttt{norm\_suzuki4\_le\_childs\_near\_zero}.}
  ...
  \item \textbf{Fourth-order convergence of $S_4$} (\cref{sec:s4-convergence}):
	...
	%\claude{CORRECTNESS: (i) ``provably strictly tighter'' --- Lean proves only $\le$ (see the abstract's note). (ii) ``for $n$ beyond an \emph{explicit} threshold'' --- in \texttt{suzuki4\_total\_error\_quartic} both $C$ and $N$ are existentially quantified; nothing explicit is recorded. (iii) ``finite-$t$'' for the $\tau^7$ bound --- \texttt{norm\_suzuki4\_level4\_uniform} is $\exists \delta > 0, \forall \tau < \delta$, so it says nothing at any \emph{given} $t$; ``existential-$\delta$ finite-$t$'' was self-contradictory.}
\end{enumerate}
...
the optional Level~4 local small-$\tau$ refinement with an explicit
...
\begin{itemize}
  \item To our knowledge, the \emph{first machine-checked convergence proof}
    of the Lie--Trotter product formula---the $n \to \infty$ limit
    statement---and the first machine-checked \emph{higher-order}
    product-formula bounds (Strang, Suzuki $S_4$) and multi-operator
    extensions in any proof assistant. A first-order Trotterization
    step-error lemma was previously mechanized in Rocq inside the QBlue
    verified compiler~\cite{liVerifiedCompilerQuantum2025}.
...
To our knowledge, no prior proof-assistant development contains the
Lie--Trotter \emph{convergence} statement, a higher-order product-formula
bound, or commutator-scaling Trotter bounds; the closest prior mechanization
is QBlue's first-order step-error lemma in
Rocq~\cite{liVerifiedCompilerQuantum2025} (\cref{sec:bg-lean}).
